...
public class DerivationChainSearchServiceTest {	
	...
	public void findTimeSeriesUpchainTest() {
		...
		List<TimeSeriesDescription> result = service.findTimeSeriesInDerivationChain("", "upchain", null, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY, DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "upchain", true, true, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH));
		result = service.findTimeSeriesInDerivationChain("", "upchain", true, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY));
		result = service.findTimeSeriesInDerivationChain("", "upchain", null, true, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PUBLISH));
		result = service.findTimeSeriesInDerivationChain("", "upchain", false, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "upchain", null, false, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "upchain", false, false, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "upchain", null, null, DESC_PRIMARY_PUBLISH.getParameter(), null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY, DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "upchain", null, null, "Incorrect", null, null, testStart, testEnd, false);
		assertTrue(result.isEmpty());
		result = service.findTimeSeriesInDerivationChain("", "upchain", true, true, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH));
	}
	...
	public void findTimeSeriesDownchainTest() {
		...
		List<TimeSeriesDescription> result = service.findTimeSeriesInDerivationChain("", "downchain", null, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY, DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, true, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH));
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY));
		result = service.findTimeSeriesInDerivationChain("", "downchain", null, true, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PUBLISH));
		result = service.findTimeSeriesInDerivationChain("", "downchain", false, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", null, false, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", false, false, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", null, null, DESC_PRIMARY_PUBLISH.getParameter(), null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH, DESC_PRIMARY, DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", null, null, "Incorrect", null, null, testStart, testEnd, false);
		assertTrue(result.isEmpty());
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, true, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH));
	}
	...
	public void findTimeSeriesUpchainDownchainTest() {
		...
		result = service.findTimeSeriesInDerivationChain("", "upchain", true, true, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PRIMARY_PUBLISH));
		result = service.findTimeSeriesInDerivationChain("", "upchain", false, false, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertTrue(result.isEmpty());
		result = service.findTimeSeriesInDerivationChain("", "downchain", null, null, null, null, null, testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_PUBLISH, DESC_NEITHER));
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, true, null, null, null, testStart, testEnd, false);
		assertTrue(result.isEmpty());
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, null, null, null, null, testStart, testEnd, false);
		...
		result = service.findTimeSeriesInDerivationChain("", "downchain", true, true, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertTrue(result.isEmpty());
		result = service.findTimeSeriesInDerivationChain("", "downchain", false, false, DESC_PRIMARY_PUBLISH.getParameter(), DESC_PRIMARY_PUBLISH.getComputationIdentifier(), DESC_PRIMARY_PUBLISH.getComputationPeriodIdentifier(), testStart, testEnd, false);
		assertThat(result, containsInAnyOrder(DESC_NEITHER));
	}
}